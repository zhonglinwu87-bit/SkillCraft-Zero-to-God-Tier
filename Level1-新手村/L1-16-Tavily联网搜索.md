...
> 💡 **白话小课堂**：跟 L1-01 只看一个脚本不同，这个 Skill 的头部多了一个关键字段 `requires: { env: ["TAVILY_API_KEY"] }`——它在告诉系统"没有这个 API Key 就别加载我"。
...
`.mjs` = ES Module JavaScript（ES = ECMAScript，JavaScript 的国际标准名称）。相比 `.js`：
