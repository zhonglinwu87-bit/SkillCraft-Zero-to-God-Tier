...
> ⚠️ **避坑指南**： 安装前确保 Node.js 版本是 18 以上。你可以在命令行输入 `node -v` 查看版本。
...
> 💡 **白话小课堂**：`npx skills` 是"万能安装器"——不管你用哪个AI平台，它都能装。就像USB-C转接头，插什么设备都能用。
...
> 💡 **白话小课堂**：搭 Skill 环境就像**装修一个工作间**——桌子（Claude Code）、五金超市供应链（marketplace）、工具箱（Skill）、试用（测试）。环境搭好了，后面的路才走得顺。
