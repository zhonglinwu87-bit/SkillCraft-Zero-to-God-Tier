...
```
Skill = YAML 元数据 + Markdown 指令 + 可选资源文件
...
它是：
  ...
  ✓ 在 AI 处理用户请求之前，先被加载到 AI 的上下文中
  ✓ 像一个"预制的高质量 System Prompt 片段"
```
...
```
训练（Fine-tuning）：      改变模型的神经网络权重
Skill（Agent Skills）：    改变模型的输入文字
...
## 3. 三级加载系统（Progressive Disclosure）

这是 Skill 设计中最精妙的部分。理解它，你才理解为什么 SKILL.md 被设计成现在这个样子。
...
> **描述**：当用户想要撰写、重写或改进任何页面的营销文案时使用——
> 包括首页、落地页、定价页、功能页。当用户说"写文案"、"改进这段文案"、
> "重写这个页面"、"营销文案"、"标题帮助"、"CTA文案"、"价值主张"时触发。
